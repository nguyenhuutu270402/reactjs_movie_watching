  ...
  return (
    <div className=''>
      <div className='flex flex-wrap items-center justify-between gap-4'>
        <div className='flex flex-wrap items-center justify-between gap-2 md:hidden'>
          <div className='flex w-full items-center justify-between sm:col-span-4'>
            <h1 className='text-sm font-normal text-[#246CF9] sm:text-base'>
              {selectedRowKeys.length} Clients selected
            </h1>
            <Button
              label='Export'
          ...
          <div className='flex  w-full  items-center gap-2'>
            ...
            <Input
              ...
              classNameIcon='mr-[10px] h-4 w-4  md:h-5 md:w-5'
              classNameForm='flex  w-full items-center justify-around rounded-[10px] bg-[#1E1F25] p-[13px] text-[#757B8C] '
              className='w-full grow border-none bg-transparent text-[10px] text-[#757B8C] outline-none md:text-[16px]'
            />
          </div>
        </div>
      </div>
      ...
      <div className='flex flex-col pb-4 '>
        <div className={`my-4 ${dataSource.length >= 1 ? `h-[calc(100vh-380px)] ` : `h-full`}  overflow-x-auto pb-4`}>
          <Table
            ...
            columns={[
              ...
              },
              {
                ...
                width: 180,
                render: ({ time_start_wifi, time_start_cellular }) => {
                  return (
                    <>
                      <ConnectionTime time={time_start_wifi} />
                      <ConnectionTime time={time_start_cellular} />
                    </>
                  )
                }
              },
        ...
        <BlockAndError
          name='Confirmation'
          title_one='Are you sure want to Block'
          title_two='this user?'
...
// const ConnectionTimeItem = ({ time }: { time: string }) => {
//   const [minutes, setMinutes] = useState<number>(moment().diff(time, 'minutes'))

//   useEffect(() => {
//     const interval = setInterval(() => {
//       setMinutes(moment().diff(time, 'minutes'))
//     }, 1000)

//     return () => clearInterval(interval)
//   }, [time])

//   return <div>{convertMinutesToHoursAndDays(minutes)}</div>
// }
